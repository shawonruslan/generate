...
def patch_android():
    ...
    for name in ("build.gradle.kts", "build.gradle"):
        ...
        # Plugins such as flutter_plugin_android_lifecycle are compiled against
        # SDK 35, so the app has to compile against 35 as well (backwards
        # compatible - targetSdk stays whatever the template chose).
        text = re.sub(r"compileSdk(?:Version)?\s*=?\s*flutter\.compileSdkVersion", "compileSdk = 35", text)
        text = re.sub(r"compileSdk(?:Version)?\s*=\s*\d+", "compileSdk = 35", text)
        if text != original:
            save(gradle, text, "minSdk 23 + compileSdk 35")
        break
