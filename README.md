...
queue - a 1:1 port of the *Automation Hub - Unified Zedge Content Studio* web
dashboard, with a professional gold/ink UI. One codebase builds:
...
## What is inside (feature parity with the HTML dashboard)

| Tab | Features |
| --- | --- |
| **Dashboard** | Live stats per account (queued, wallpapers, special sets, audio, done, failed), today's type + remaining quota, recent uploads with phone preview, holiday strip |
| **Upload Queue** | Single JPEG/MP3 upload, 24H (x4) / Dual (x2) / Battery (x4) wallpaper sets, Live wallpaper + Charging animation videos (50 MB, optional cover), auto resize to 1620x2880 JPEG, R2 upload, duplicate guard, filter pills, search, pagination, item editor (title/tags/category/description, pin date, requeue, copy to other accounts, delete, audio play, phone preview with 9 device frames) |
| **Calendar** | Real 28-day planner: Dhaka-time day type cycle (WALLPAPER > 24H > DUAL > BATTERY > LIVE > CHARGING), 3 slots/day, pinned + fallback rows, holidays/special days from Nager.date (Europe/Americas/Asia) + Google Calendar (IN/BD) + built-in world days, drag & drop tray, tap-to-pin day sheet, real internet time sync |
| **Distribute** | Bulk round-robin upload across Zedge 1 / 2 / 3 with set grouping in file-name order, plan preview, progress, per-node counters, persisted pointer |
| **GitHub Control** | `generator.yml` dispatcher: Ringtone generator, Metadata generator, Gemini session (validate / attach), workflow runs (refresh / open / delete / clean completed), repository file browser (push up to 25 MB, delete), activity log; settings synced to Zedge 1 RTDB |
| **Settings** | Google Calendar API key, holiday regions, preview device, clock re-sync, cache cleanup, about |

All three Firebase Realtime Databases are reached over REST + Server-Sent
Events, so the app runs identically on Windows and Android without Firebase
plugins.
