...
- Build: GitHub Actions → **Desktop (Windows) - Meta Hawladar** → artifacts `MetaHawladar-windows-msi`, `MetaHawladar-windows-exe`, `MetaHawladar-windows-portable`.
- **Installer error 2503 / 2502** on a new PC: v31 installs per-machine (UAC prompt), which avoids the error. If it still appears,
  run `Install-MetaHawladar.bat` (shipped next to the MSI) as administrator, or use the portable zip — no installer needed.
  Full steps in `desktop/installer/INSTALL-README.txt`.
- **Today's uploads** cards: all accounts are always visible on any window size (cards share the row while ≥220dp each,
  otherwise they wrap to a balanced grid, e.g. 4 accounts → 2×2). No horizontal scrolling.
