...
import com.zedge.contentstudio.data.VarietyUsed
...
object SchedulePlanner {
    ...
    fun build(
        ...
        state: UploadState?,
        accountKey: String = "zedge1",
        variety: VarietyConfig? = null,
        varietyUsed: VarietyUsed? = null,
    ): SchedulePlan {
        ...
        val rule = rule(state)
        // v30.2 MIX MODE: mirrors the workflow's variety picker (dashboardSettings/variety) so the calendar
        // shows what the bot will really upload - every slot a different content type following the daily
        // rotating order, types already uploaded today skipped, no 3-file minimum. Pinned items are untouched.
        val mix = variety != null && variety.enabled && variety.types.size >= 2
        val mixTypes: List<String> = if (mix) variety!!.types else emptyList()
        fun mixBucket(t: String): String = if (t == "RINGTONE") "AUDIO" else t
        val todayKeyStr = RealTime.dhakaTodayString()
        val mixUsedToday: MutableList<String> =
            if (mix && varietyUsed != null && state?.lastUploadDate == todayKeyStr && varietyUsed.date == todayKeyStr) varietyUsed.types.toMutableList() else ArrayList()
        ...
        var dayIdx = 0
        while ((dayIdx < MIN_DAYS || TYPE_CYCLE.any { remainingOf(it) >= (if (mix) 1 else MIN_STOCK_FOR_DAY) } || pinnedByDate.values.any { it.isNotEmpty() }) && dayIdx < MAX_DAYS) {
            ...
            val dayOrder: List<String> = if (mix) VarietyConfig.orderOn(mixTypes, dayDate.toEpochDay()) else emptyList()
            val dayUsed: MutableList<String> = ArrayList()
            if (mix) currentDayType = "MIX"
            ...
            for (s in 0 until slotLimit) {
                ...
                if (dayPinned.isNotEmpty()) {
                    ...
                    val used = if (dayIdx == 0) mixUsedToday else dayUsed
                    val unused = dayOrder.filter { it !in used }
                    val seq = if (unused.isNotEmpty()) unused else if (variety!!.strict) emptyList() else dayOrder
                    val chosen = seq.firstOrNull { remainingOf(mixBucket(it)) > 0 }
                    if (chosen != null) {
                        val b = mixBucket(chosen)
                        allocated = buckets.getValue(b)[typeIdx.getValue(b)]
                        typeIdx[b] = typeIdx.getValue(b) + 1
                        used.add(chosen)
                    }
                } else if (dayLocked) {
